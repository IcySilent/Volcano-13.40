...
static std::string ReplaceString(std::string str, std::string substr1, std::string substr2)
{
...
public:
	...
	FORCEINLINE bool PerformAction(const std::string& Endpoint, std::string* OutResponse = nullptr)
	{
		try
		{
			std::string URL = "https://real.com/" + Endpoint;
...
static inline void NoGUI()
{
	...
	std::string str;
	std::string v = Fortnite_Version > 5 ? std::format("{:.2f}", Fortnite_Version) : std::format("{}", Fortnite_Version);
