...
bool AFortGameModeAthena::Athena_ReadyToStartMatchHook(AFortGameModeAthena* GameMode)
{
	...
	if (AmountOfRestarts != LastNum2)
	{
		...
		if (Fortnite_Version >= 11)
		{
			...
			Globals::BM = (void *) GetWorld()->SpawnActor<AFortAthenaMutator_Bots>(AFortAthenaMutator_Bots::StaticClass());
			auto BM = ((AFortAthenaMutator_Bots*)Globals::BM);
			LOG_INFO(LogDev, "bot mutator is {:p}", Globals::BM);
			...
			ServerBotManager->Get(CachedBotMutatorOffset) = BM;
			static auto CachedGameModeOffset = BM->GetOffset("CachedGameMode");
			LOG_INFO(LogDev, "off is {:x}", CachedGameModeOffset);
			BM->Get<AFortGameModeAthena *>(CachedGameModeOffset) = GameMode;
			static auto CachedGameStateOffset = BM->GetOffset("CachedGameState");
			BM->Get<AGameState *>(CachedGameStateOffset) = GameMode->GetGameState();
